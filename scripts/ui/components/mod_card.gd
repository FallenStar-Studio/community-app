...
func _ready() -> void:
	...
	add_theme_stylebox_override("hover", _card_style(Color("242c36"), Tokens.AMBER, 5, 6))
	add_theme_stylebox_override("pressed", _card_style(Color("3b342c"), Tokens.AMBER, 0, 3))
...
func setup(value: Dictionary) -> void:
	record = value
	custom_minimum_size = Vector2(Tokens.CARD_MIN_WIDTH, 170)
	...
	var kind := I18n.field(record, "type")
	var kind_tag := _tag(kind, Tokens.CYAN, 88)
	meta.add_child(kind_tag)
	meta.add_spacer(false)
	var is_demo := bool(record.get("demo_only", record.get("is_demo", true)))
	meta.add_child(_tag(I18n.text("demo_no_download") if is_demo else I18n.source_label(record), Tokens.AMBER if is_demo else Tokens.CYAN, 112))

	var heading := _label(I18n.field(record, "name"), 16, Color("eef5ff"), true)
	heading.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	heading.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	stack.add_child(heading)
	var excerpt := _label(I18n.field(record, "description"), 11, Color("aab8cd"), true)
	excerpt.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	excerpt.max_lines_visible = 3
	...
	stack.add_child(_label("%s  ·  %s" % [I18n.field(record, "author"), I18n.field(record, "version")], 9, Color("8191aa"), true))
